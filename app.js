...
async function archiveGuideDocument({ title, fileName, fileBuffer, mimeType = 'application/octet-stream', uploadedBy = 'admin' }) {
  ...
  const result = await run(
    'INSERT INTO guide_documents (title, fileName, relativePath, mimeType, sizeBytes, uploadedBy, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [safeTitle, finalFileName, relativePath, safeMimeType, sizeBytes, safeUploadedBy, now, now]
  );
